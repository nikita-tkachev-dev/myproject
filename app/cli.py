...
def init_cli(app: Flask):
    ...
    def seed_exercises():
        ...
        click.echo("🌱 Seeding exercises...")

        # Check if exercises already exist
        if Exercise.query.count() > 0:
            if not click.confirm("⚠️  Exercises already exist. Clear and reseed?"):
        ...
        try:
            ...
            db.session.commit()
            click.echo(f"✅ Successfully seeded {len(exercises)} exercises!")
            ...
            for group in ["legs", "back", "chest", "core", "shoulders", "arms"]:
                ...
                click.echo(f"   {group.capitalize()}: {count} exercises")

        except Exception as e:
            db.session.rollback()
            click.echo(f"❌ Error seeding exercises: {str(e)}", err=True)

    @app.cli.command("seed-all")
    def seed_all():
        """Seed all data (exercises, templates, etc.)"""
